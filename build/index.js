...
function App() {
  const [gameState, setGameState] = import_react.useState("WAITING");
  const [round, setRound] = import_react.useState(1);
  const [player1Word, setPlayer1Word] = import_react.useState("");
  const [player2Word, setPlayer2Word] = import_react.useState("");
  const [score, setScore] = import_react.useState(0);
  const [timeLeft, setTimeLeft] = import_react.useState(20);
  const calculateScore = (word1, word2) => {
    if (!word1 || !word2)
      return 0;
    const maxLength = Math.max(word1.length, word2.length);
    const diffLetters = Array.from({ length: maxLength }).filter((_, i) => word1[i] !== word2[i]).length;
  ...
  const handleNextState = () => {
    switch (gameState) {
      ...
      case "COUNTDOWN":
        ...
        break;
      case "ACTIVE":
        const roundScore = calculateScore(player1Word, player2Word);
      ...
      /* @__PURE__ */ jsx_dev_runtime.jsxDEV("main", {
        children: [
          ...
          /* @__PURE__ */ jsx_dev_runtime.jsxDEV("button", {
            className: "next-state-btn",
            onClick: handleNextState,
            children: gameState === "WAITING" ? "START GAME" : gameState === "COUNTDOWN" ? "START ROUND" : gameState === "ACTIVE" ? "END ROUND" : gameState === "SCORING" ? "NEXT ROUND" : "PLAY AGAIN"
          }, undefined, false, undefined, this)
        ]
      }, undefined, true, undefined, this)
    ]
